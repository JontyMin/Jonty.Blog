...
namespace Jonty.Blog.Application.Tencent.Impl
{
    public class MTAService:JontyBlogApplicationServiceBase,IMTAService
    {
        private readonly IHttpClientFactory _httpClient;

        public MTAService(IHttpClientFactory httpClient)
        {
        ...
        {
            var url = $"{api}{GgenerateMTAQuery(keyValues)}";

            using var client = _httpClient.CreateClient();
            var result = await client.GetStringAsync(url);

            return result.FromJson<dynamic>();
        }
        ...
        /// <param name="keyValues"></param>
        /// <returns></returns>
        private string GgenerateMtaSign(Dictionary<string, string> keyValues)
        {
            ...
            return (MtaConfig.SECRET_KEY + sb.ToString()).EncodeMd5String().ToLower();
        }
        ...
        /// <param name="keyValues"></param>
        /// <returns></returns>
        private string GgenerateMTAQuery(Dictionary<string, string> keyValues)
        {
            var sign = GgenerateMtaSign(keyValues);
